...
contract PRESALE is Ownable, ReentrancyGuard {
   ...
   function _executeFinalizationSteps() external {
    ...
    ) = calculateBNBLiquidityParams(accumulatedBNB, tokensReceived);

    // 步骤3: 设置流动性参数（如果未配置）
    if (liquidityTokenAmount == 0 || liquidityBNBAmount == 0) {
        liquidityTokenAmount = tokenAmount;
        liquidityBNBAmount = bnbAmountForLP;
        slippageProtection = 500; // 默认5%滑点保护
    }
    ...
    function configureLiquidity(
        ...
        if (_tokenAmount == 0) {
            ...
            require(
                tokensReceived > 0,
                "Failed to receive tokens from factory"
            );
    ...
    // 🔧 新增：检查工厂授权状态
    function getFactoryAllowance() internal view returns (uint256) {
        if (factoryAddress == address(0) || coinAddress == address(0)) {
            return 0;
        }
 ...
 * @return receivedAmount 实际获得的代币数量
 */
// 注意：receiveTokensFromFactory 函数已在前面定义，这里删除重复定义
     ...
     * @return 剩余授权额度
     */
    // 注意：getFactoryAllowance 函数已在前面定义，这里删除重复定义
